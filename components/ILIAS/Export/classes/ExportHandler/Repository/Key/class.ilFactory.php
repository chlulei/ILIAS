...
class ilFactory implements ilExportHandlerRepositoryKeyFactoryInterface
{
    ...
    public function handlerFromElement(
        ilExportHandlerRepositoryElementInterface $element
    ): ilExportHandlerRepositoryKeyInterface {
        return $this->handler()
            ->withResourceId($element->getResourceId()->serialize() ?? ilExportHandlerRepositoryKeyInterface::EMPTY_RESOURCE_IDENTIFICATION)
            ->withObjectId($element->getObjectId() ?? new ObjectId(ilExportHandlerRepositoryKeyInterface::EMPTY_OBJECT_ID));
    }
